...
#define DEBUG(l,f,v...) ;
#else
#define DEBUG(l,f,v...) { if(l<=devAoEurotherm2kDebug) printf(f,## v); }
#endif
volatile int devAoEurotherm2kDebug = 0;
}

#define HEXCHAR2INT(c) ((c)>'9' ? 10+tolower(c)-'a' : tolower(c)-'0')

class AoEurotherm2k : public DevMpf
{
public:
	AoEurotherm2k(dbCommon*,DBLINK*);
...
MAKE_LINCONV_DSET(devAoEurotherm2k,AoEurotherm2k::dev_init)

long AoEurotherm2k::dev_init(void* v)
{
	aoRecord* pr = (aoRecord*)v;
	new AoEurotherm2k((dbCommon*)pr,&(pr->out));
	return 0;
}

AoEurotherm2k::AoEurotherm2k(dbCommon* pr,DBLINK* l) : DevMpf(pr,l,false)
{
	...
	DEBUG(2,"AoEurotherm2k::AoEurotherm2k(%s)\n",pr->name);
	...
	if (pio->parm) {
		DEBUG(2,"AoEurotherm2k::AoEurotherm2k pio->parm = '%s'\n", pio->parm);
	...
	DEBUG(2,"AoEurotherm2k::AoEurotherm2k term = '%s'\n", term);
	DEBUG(2,"AoEurotherm2k::AoEurotherm2k buffer_start_index = %d\n", buffer_start_index);
	DEBUG(2,"AoEurotherm2k::AoEurotherm2k timeout = %d\n", timeout);
	DEBUG(2,"AoEurotherm2k::AoEurotherm2k format = %s\n", format);
}

long AoEurotherm2k::startIO(dbCommon* pr)
{
	aoRecord* ao = (aoRecord*)pr;
	...
	DEBUG(2,"AoEurotherm2k::startIO(%s)\n",pr->name);
...
long AoEurotherm2k::completeIO(dbCommon* pr,Message* pm)
{
	DEBUG(2,"AoEurotherm2k::completeIO(%s)\n",pr->name);
	Char8ArrayMessage *pcm = (Char8ArrayMessage *)pm;;
	int rc=0;

	DEBUG(2,"AoEurotherm2k::completeIO(%s)\n",pr->name);
	if ((pm->getType()) != messageTypeChar8Array) {
		epicsPrintf("%s DevAoEurotherm2k::completeIO illegal message type %d\n",
			pr->name,pm->getType());
		rc = -1;
	} else {
		if(pcm->status!=0) rc=-1;
	}
	if (rc != 0) DEBUG(2,"AoEurotherm2k::completeIO(%s), error=%d\n",
                                            pr->name, rc);
